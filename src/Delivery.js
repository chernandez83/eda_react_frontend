...
const Delivery = (props) => {
    /*const [state, setState] = useState({
        "id":"01GDH4QE93NWPVF5G0E3WJMY08",
        "budget":100,
        "notes":"Buy 4 burguers",
        "status":"ready"
    });*/
    const [state, setState] = useState('');
    ...
    return <div className="row w-100">
        <div className="col-12 mb-4">
            <h4 className="bold text-white">Delivery {state.id}</h4>
        </div>
        <div className="col-12 mb-5">
            <div className="progress">
                {state.status !== 'ready' ? <div className={state.status === 'active' ? "progress-bar bg-success progress-bar-striped progress-bar-animated" 
                                                                                        : "progress-bar bg-success"}
                    role="progressbar" style={{width: '50%'}}>
                </div> : ''}
                {state.status === 'collected' || state.status === 'completed' ? <div className={state.status === 'collected'
